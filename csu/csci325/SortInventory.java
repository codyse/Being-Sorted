...
public class SortInventory {
    ...
    public static void sortByAmountAcend(Inventory currInventory) {
        ...
        for (int i = 0; i < currInventory.items.size(); i++) {
            for (int j = i + 1; j < currInventory.items.size(); j++) {
                if (currInventory.items.get(i).getAmount() > currInventory.items.get(j).getAmount()) {
    ...
    public static void sortByAmountDecend(Inventory currInv) {
        ...
        for (int i = 0; i < currInv.items.size(); i++) {
            for (int j = i + 1; j < currInv.items.size(); j++) {
                if (currInv.items.get(i).getAmount() < currInv.items.get(j).getAmount()) {
    ...
    public static void switchPosition(Inventory currInv, String firstItem, String secItem) {
        String tempOne = "";
        String tempTwo = "";
        int tempAmountOne = 0;
        int tempAmountTwo = 0;
        ...
        for (int i = 0; i < currInv.items.size(); i++) {
            ...
            if (i == currInv.items.size() - 1 && !currInv.items.get(i).getName().equals(firstItem)) {
                System.out.println("Error! Can't find " + firstItem + " in our " + "inventory.");
            }
        }
        ...
        for (int i = 0; i < currInv.items.size(); i++) {
            ...
            if (i == currInv.items.size() - 1 && !currInv.items.get(i).getName().equals(secItem)) {
                System.out.println("Error! Can't find " + secItem + " in our " + "inventory.");
            }
        }
